...
const CustomDot = (props) => {
  ...
  if (milestoneIndices.includes(index)) {
    return (
      <circle 
        cx={cx} cy={cy} r={4} 
        stroke="#333" strokeWidth={2} fill="white" 
      />
    );
  }
  return null;
...
const DepartmentCompletionChart = () => {
  ...
  return (
    <div className="w-full bg-sidebar p-4 ">
      <div className="flex flex-col md:flex-row justify-between md:items-center mb-8">
        <h2 className="text-xl md:text-2xl font-semibold text-gray-800">Monthly Completion Trends</h2>
        <div className="flex gap-4 text-sm font-medium text-gray-500">
          <button className="flex items-center gap-2" onClick={() => setShowCompleted(!showCompleted)}>
  <span className="w-3 h-3 rounded-full bg-[#7C3AED]" /> 
  <span className={`${!showCompleted ? 'line-through text-gray-500' : ''}`}>Completed</span>
</button>

          <button className="flex items-center gap-2" onClick={() => setShowPending(!showPending)}>
            <span className="w-3 h-3 rounded-full bg-[#C4B5FD]" /> 
            <span className={`${!showPending ? 'line-through text-gray-500' : ''}`}>Pending</span>
          </button>
        </div>
      </div>
